...
int main()
{
    ...
    nodal_coords.clear();
    nodalCoords(128,M,nodal_coords);
    ...
    Eigen::SparseMatrix<double> A(M.nNodes+1,M.nNodes+1);
    Eigen::VectorXd b(M.nNodes+1);
    ...
    for(int elm_num = 1; elm_num<=M.nElements;elm_num++)
    {
        ...
        for(int i=0;i<3;i++)
        {
            for(int j=0;j<3;j++)
            {
                tripletList.push_back(T(Local2GlobalMap(M,elm_num,i+1),Local2GlobalMap(M,elm_num,j+1),Ke[i][j]));
            }
        }
